...
export default function SignupForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(getAuthErrorMessage(err, "Impossible de créer le compte."));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex w-full max-w-sm flex-col gap-6">
      <header className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tighter text-zinc-900">Créer un compte</h1>
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          id="signup-email" type="email" autoComplete="email" required value={email}
          onChange={(e) => setEmail(e.target.value)}
          aria-invalid={Boolean(error)} aria-describedby={error ? "signup-auth-error" : undefined}
      ...
      <PasswordField
        id="signup-password" label="Mot de passe" value={password}
        onChange={(e) => setPassword(e.target.value)} autoComplete="new-password"
        required minLength={6} ariaInvalid={Boolean(error)} ariaDescribedBy={error ? "signup-auth-error" : undefined}
      />
      <PasswordField
        id="signup-password-confirm" label="Confirmer le mot de passe" value={confirmPassword}
        onChange={(e) => setConfirmPassword(e.target.value)} autoComplete="new-password"
        required minLength={6} ariaInvalid={Boolean(error)} ariaDescribedBy={error ? "signup-auth-error" : undefined}
      />

      {error && (
        <p id="signup-auth-error" role="alert" className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">
          {error}
        </p>
      )}
